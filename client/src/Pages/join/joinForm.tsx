...
interface JoinObserver {
  update: (isJoinSuccessful: boolean) => void;
}
...
const JoinForm: FunctionComponent<JoinFormProps> = ({ onError }) => {
  ...
  const [observers, setObservers] = useState<JoinObserver[]>([]);
  ...
  const registerObserver = (observer: JoinObserver) => {
    setObservers((oldObservers) => [...oldObservers, observer]);
  };

  const unregisterObserver = (observer: JoinObserver) => {
    setObservers((oldObservers) => oldObservers.filter((o) => o !== observer));
  };
